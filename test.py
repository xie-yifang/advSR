...
def test(
    ...
    save_image = False
    save_image_dir = "D:/musubject/output/SR/TEST_1"
    ...
    with torch.no_grad():
        ...
        while batch_data is not None:
            ...
            # --------------------------------------------
            if sr.size() != gt.size():
                sr = torch.nn.functional.interpolate(sr, size=(gt.size(2), gt.size(3)), mode='bicubic',
                                                     align_corners=False)
            ...
            if save_image:
                ...
                # 保存图像
                sr_image_pil.save(os.path.join(save_image_dir, image_name))
...
def main() -> None:
    ...
    parser.add_argument("--config_path",
                        type=str,
                        default="./configs/test/ESRGAN_x2-imagenet.yaml",
                        required=False,
