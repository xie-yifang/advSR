...
from imgproc import random_crop_torch, random_rotate_torch, random_vertically_flip_torch, random_horizontally_flip_torch, preprocess_imagenet
...
def main():
    ...
    parser.add_argument("--config_path",
                        type=str,
                        default="./configs/train/ESRGAN_x2-DIV2K.yaml",
                        required=False,
    ...
    # g_model, ema_g_model, d_model, vgg19_model = build_model(config, device)

    g_model, ema_g_model, d_model, vgg19_model = build_model(
        config, device)
    ...
    # G model是生成器模型 将低分辨率图像转换为高分辨率图像
    # D model（判别器模型） 判别器模型D的任务是区分生成的超分辨率图像和真实的高分辨率图像
    ...
    for epoch in range(start_epoch, config["TRAIN"]["HYP"]["EPOCHS"]):
        ...
        paired_test_data_prefetcher.reset()  # 重置测试数据prefetcher的指针
        test_batch = paired_test_data_prefetcher.next()  # 获取一批测试数据
        test_lr = test_batch['lr'].to(device, non_blocking=True)  # 假设测试数据包含lr键

        with torch.no_grad():
            test_sr = g_model(test_lr)  # 使用模型进行重建

        # 处理并保存重建后的图像
        sample_png_dir = os.path.join(samples_dir, "picture_senet")
        make_directory(sample_png_dir)
        grid_image = torchvision.utils.make_grid(test_sr, nrow=1)  # 创建网格图像
        save_image_path = os.path.join(sample_png_dir, f'reconstructed_epoch_{epoch}.png')
        save_image(grid_image, save_image_path)  # 保存图像
...
def build_model(
    ...
    vgg19_model.eval()
    return g_model, ema_g_model, d_model, vgg19_model
...
def define_loss(config: Any, device: torch.device) -> [nn.L1Loss, model.ContentLoss, nn.BCEWithLogitsLoss]:
    ...
    elif config["TRAIN"]["LOSSES"]["PIXEL_LOSS"]["NAME"] == "L2Loss":
...
def train(
    ...
    while batch_data is not None:
        ...
        with amp.autocast():
            ...
            preprocessed_gt = preprocess_imagenet(gt)
            preprocessed_sr = preprocess_imagenet(sr)
            with torch.no_grad():
                cacategory_logit_gt = vgg19_model(preprocessed_gt)
                category_logit_sr = vgg19_model(preprocessed_sr)
            gt_output = d_model(gt.detach().clone())  # category_logit_gt
            sr_output = d_model(sr)  # category_logit_sr
            pixel_loss = pixel_criterion(sr, gt)
            content_loss = content_criterion(sr, gt, category_logit_sr, cacategory_logit_gt)
    ...
    totalavg_g_loss = total_g_loss / batches
    totalavg_d_loss = total_d_loss / batches
    totalavg_pixel_loss = total_pixel_loss / batches
    totalavg_adversarial_loss = total_adversarial_loss / batches
    totalavg_content_loss = total_content_loss / batches
    writer.add_scalar("Epoch/G_loss", totalavg_g_loss, epoch)
    writer.add_scalar("Epoch/D_loss", totalavg_d_loss, epoch)
    writer.add_scalar("Epoch/pixel_loss", totalavg_pixel_loss, epoch)
    writer.add_scalar("Epoch/adversarial_loss", totalavg_adversarial_loss, epoch)
    writer.add_scalar("Epoch/content_loss_loss", totalavg_content_loss, epoch)
...
if __name__ == "__main__":
    ...
    main()
    end_time = time.time()
    total_training = start_time - end_time
    print(f"total train time: {total_training:.2f} seconds")
