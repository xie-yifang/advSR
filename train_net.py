...
def main():
    ...
    parser.add_argument("--config_path",
                        type=str,
                        default="./configs/train/RRDBNet_x2-DIV2K.yaml",
    ...
    for epoch in range(start_epoch, config["TRAIN"]["HYP"]["EPOCHS"]):
        ...
        paired_test_data_prefetcher.reset()  # Reset the pointer of the test data prefetcher
        test_batch = paired_test_data_prefetcher.next()  # Fetch one batch of test data
        test_lr = test_batch['lr'].to(device, non_blocking=True)  # Assume the test data contains an 'lr' key

        with torch.no_grad():
            test_sr = g_model(test_lr)  # Perform reconstruction using the model

        # Process and save the reconstructed image
        sample_png_dir = os.path.join(samples_dir, "picture_seblock")
        make_directory(sample_png_dir)
        grid_image = torchvision.utils.make_grid(test_sr, nrow=1)  # Create a grid image
        save_image_path = os.path.join(sample_png_dir, f'reconstructed_epoch_{epoch}.png')
        save_image(grid_image, save_image_path)  # Save the image
