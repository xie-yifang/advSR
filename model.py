...
class RRDBNet(nn.Module):
    def __init__(
        ...
        # Add SENet
        self.se_res_block = SEResBlock(channels)

        # Convolutional block after the feature extraction network.
        ...
        # Convolutional block after upsampling.
        ...
        # Add SEBlock option 1
        # self.se_block = SEBlock(channels)
    # Torch.script compatible forward
    # ------------------------------------------------------------
    def _forward_impl(self, x: Tensor) -> Tensor:
        conv1 = self.conv1(x)
        # Optional: add SEBlock here
        # conv1 = self.se_block(conv1)

        x = self.trunk(conv1)

        # Option: add SENet position 2
        # x = self.se_res_block(x)

        x = self.conv2(x)
        x = torch.add(x, conv1)

        # Option: add SENet again
        x = self.se_res_block(x)
...
class _ResidualDenseBlock(nn.Module):
    ...
    Paper: `Densely Connected Convolutional Networks <https://arxiv.org/pdf/1608.06993v5.pdf>`

    Args:
        channels (int): Number of input channels.
        growth_channels (int): Number of channels added in each convolutional layer.
    """
...
class DiscriminatorForVGG(nn.Module):
    def __init__(
        ...
        self.classifier = nn.Sequential(
            # For ImageNet dataset size is 15
            # nn.Linear(int(8 * channels) * 15 * 15, 100),
...
# Modification 1: content_loss added category loss
class ContentLoss(nn.Module):
    ...
    def forward(self, sr_tensor: Tensor, gt_tensor: Tensor, category_logit_sr, category_logit_gt) -> [Tensor]:
        ...
        # Based on L1 norm
        content_losses = [F_torch.l1_loss(sr_feature[self.feature_extractor_nodes[i]],
                                          gt_feature[self.feature_extractor_nodes[i]]) for i in
                          range(len(self.feature_extractor_nodes))]

        # Based on L2 norm
        # content_losses = [F.mse_loss(sr_feature[self.feature_extractor_nodes[i]],
        #                              gt_feature[self.feature_extractor_nodes[i]]) for i in
        #                   range(len(self.feature_extractor_nodes))]
        ...
        # Get predicted labels for generated image
        # category_logit_sr = model(sr_tensor)
        _, predicted_labels_sr = torch.max(F.softmax(category_logit_sr, dim=1), 1)

        # Get true labels for target image
        # category_logit_gt = model(gt_tensor)
        _, true_labels_gt = torch.max(F.softmax(category_logit_gt, dim=1), 1)

        # Compute category loss
        category_loss = self.category_loss(category_logit_sr, true_labels_gt)

        content_loss = torch.Tensor([content_losses]).to(device)
        total_loss = content_loss + self.category_loss_weight * category_loss
        return total_loss
